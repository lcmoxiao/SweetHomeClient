...
public class SingleTalkActivity extends Activity {
    ...
    private void initClick() {
        sendBtn.setOnClickListener(v -> {
            ...
            Bitmap bmp = Bitmap.createBitmap(300, 300, Bitmap.Config.ARGB_8888);
            bmp.eraseColor(Color.parseColor("#FFEC808D"));
            adapter.addItem(new MsgDateBean(bmp, inputEt.getText().toString().getBytes(), new Date().toString(), 1, 1));
            inputEt.setText("");
        });
        ...
        recordSendBtn.setOnClickListener(v -> {
            ...
            Bitmap bmp = Bitmap.createBitmap(300, 300, Bitmap.Config.ARGB_8888);
            bmp.eraseColor(Color.parseColor("#FFEC808D"));
            adapter.addItem(new MsgDateBean(bmp, "录音".getBytes(), Calendar.getInstance().getTime().toString(), 5, 5));
        });
    }
    ...
    public void matchReject() {
        ...
        if (UserInfos.isOnMatching()) {
            new Thread(() ->
            {
                MatchService.rejectMatch(UserInfos.isGroupMatching(), UserInfos.user.getUserid(), UserInfos.matchUser.getUserid(), UserInfos.groupid);
    ...
    public void onMatchReject() {
        UserInfos.usingState = UserInfos.UsingState.NULL;
        handler.postDelayed(() -> Toast.makeText(this, "匹配被取消", Toast.LENGTH_SHORT).show(), 1);
        finish();
    }
    ...
    public void onFriendWant(int userid1, int userid2) {
        ...
        handler.postDelayed(builder::show, 1);
    }

    public void onFriendAgree(int userid1, int userid2) {
        handler.postDelayed(() -> Toast.makeText(this, "好友申请成功", Toast.LENGTH_SHORT).show(), 1);
        switchFragment(3);
    }

    public void onFriendReject(int userid1, int userid2) {
        handler.postDelayed(() -> Toast.makeText(this, "好友申请被拒绝,对话结束", Toast.LENGTH_SHORT).show(), 1);
        finish();
    }
}
